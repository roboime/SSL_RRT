from Univector_Field import UVF, wraptopi
...
if __name__ == '__main__' :
    ...
    for row in Field.index:
        for column in Field.columns:
            ...
            # bx.quiver(column, row, np.cos(TUF([column, row, 0], [0, 0, np.pi/3], 5, 5, 0, 0)),  np.sin(TUF([column, row, 0], [0, 0, np.pi/3], 5, 5, 0, 0)), scale=75)
            ...
            bx.quiver(column, row, np.cos(UVF([column, row, 0], target, obstacle_vector, 3, 5, 5, 2)), np.sin(UVF([column, row, 0], target, obstacle_vector, 3, 5, 5, 3)), scale=50)
